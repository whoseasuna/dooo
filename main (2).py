...
async def gate(message: Message, require_session: bool = True) -> bool:
    ...
    if not joined:
        ...
        await message.answer_photo(
            ...
            caption=(
                f"{d('🔒 You MUST join ')}<b>{ch}</b>{d(' to use ')}"
                f"{DENIAL_FONT}{d(' Spreader Bot.\n\n')}"
                f"{d('📢 Join here: ')}{url}{d('\n\n')}"
                f"{d('Then tap ')}<b>{d('I have joined ✅')}</b>{d('.')}"
            ),
            parse_mode="HTML",
            reply_markup=kb,
            has_spoiler=True,
...
# ----------------------------------------------------------------------
# /broadcast - admin sends a message to every approved/trial user
# ----------------------------------------------------------------------
...
async def cmd_broadcast(message: Message):
    ...
    if len(parts) < 2 or not parts[1].strip():
        await message.answer(
            "📢 Usage:\n"
            "/broadcast <message>\n\n"
            "Sends the message to ALL approved and trial users."
        )
        return
    text = parts[1]
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT chat_id FROM approvals "
            "WHERE status IN ('approved', 'trial')"
        ) as cur:
