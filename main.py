...
def d(text):
    ...
    unchanged. The following are passed through verbatim:
      * HTML tags <...> and </...>
      * URLs (https://, http://, t.me/, www.)
      * Telegram command tokens like /login, /send, /cancel
    """
    ...
    while i < n:
        ch = text[i]
        # Preserve /command tokens (only at start of string or after
...
class ContactsBroadcastStates(StatesGroup):
    waiting_message = State()
    waiting_delay = State()
...
# ----------------------------------------------------------------------
# Dynamic settings (admin can change at runtime)
# ----------------------------------------------------------------------
...
def normalize_channel(value: str) -> tuple:
    """Return (username, invite_url) for the given channel identifier."""
    value = value.strip()
    if value.startswith("http"):
        url = value
        if "/+" in url or "/joinchat" in url:
            return ("", url)
        # try to extract username from https://t.me/<username>
        parts = url.rstrip("/").split("/")
        uname = parts[-1].lstrip("@") if parts else ""
        return (("@" + uname) if uname else "", url)
    if not value.startswith("@"):
        value = "@" + value
    return (value, f"https://t.me/{value.lstrip('@')}")


async def get_required_channel() -> str:
    return await get_setting("required_channel", REQUIRED_CHANNEL)


async def get_required_channel_url() -> str:
    ch = await get_required_channel()
    _, url = normalize_channel(ch)
    return url


# ----------------------------------------------------------------------
# Multi-channel join list helpers
# ----------------------------------------------------------------------
async def get_join_list() -> List[str]:
    """Return list of all required join channels/groups."""
    raw = await get_setting("join_list")
    if raw:
        try:
            return json.loads(raw)
        except Exception:
            pass
    default = await get_setting("required_channel", REQUIRED_CHANNEL)
    return [default] if default else [REQUIRED_CHANNEL]


async def add_to_join_list(ch: str) -> bool:
    """Add a channel/group to the join list. Returns True if newly added."""
    lst = await get_join_list()
    uname, _ = normalize_channel(ch)
    entry = uname if uname else ch
    if entry in lst:
        return False
    lst.append(entry)
    await set_setting("join_list", json.dumps(lst))
    return True


async def remove_from_join_list(ch: str) -> bool:
    """Remove a channel/group from the join list. Returns True if removed."""
    lst = await get_join_list()
    uname, _ = normalize_channel(ch)
    entry = uname if uname else ch
    new_lst = [c for c in lst if c != entry and c != ch]
    if len(new_lst) == len(lst):
        return False
    await set_setting("join_list", json.dumps(new_lst))
    return True
...
async def user_joined_channel(user_id: int) -> bool:
    """Check if user_id has joined ALL required channels/groups."""
    _, unjoined = await get_unjoined_channels(user_id)
    return len(unjoined) == 0


async def get_unjoined_channels(user_id: int) -> tuple:
    """Returns (all_joined: bool, unjoined: list[str])."""
    channels = await get_join_list()
    unjoined = []
    for ch in channels:
        try:
            ...
            logger.warning(f"Channel check failed for {user_id} in {ch}: {e}")
...
async def get_joined_groups(client: TelegramClient) -> list:
    """Return entities of all GROUPS (not broadcast channels) the account has joined."""
    try:
    ...
    async for dialog in client.iter_dialogs():
        ...
        if dialog.is_group:
            targets.append(ent)
        elif dialog.is_channel and getattr(ent, "megagroup", False):
            # megagroup = supergroup — counts as a group, not a broadcast channel
            targets.append(ent)
    return targets
...
async def gate(message: Message, require_session: bool = True) -> bool:
    ...
    # Authorized: check all required channels/groups
    all_joined, unjoined = await get_unjoined_channels(cid)
    if not all_joined:
        join_buttons = [
            [InlineKeyboardButton(
                text=f"📢 Join {ch}",
                url=normalize_channel(ch)[1],
            )]
            for ch in unjoined
        ]
        join_buttons.append([InlineKeyboardButton(
            text="✅ I've joined all", callback_data="check_channel"
        )])
        kb = InlineKeyboardMarkup(inline_keyboard=join_buttons)
        channels_text = "\n".join(
            f"• <b>{ch}</b> — {normalize_channel(ch)[1]}" for ch in unjoined
        )
        await message.answer_photo(
            photo=START_IMAGE_URL,
            caption=(
                f"{d('🔒 You MUST join the following to use ')}"
                f"{DENIAL_FONT}{d(' Spreader Bot:\n\n')}"
                f"{channels_text}\n\n"
                f"{d('Then tap ')}<b>{d('I have joined all ✅')}</b>{d('.')}"
            ),
            parse_mode="HTML",
            reply_markup=kb,
            has_spoiler=True,
    ...
    if require_session:
        ...
        if not session:
            url = await get_required_channel_url()
            await message.answer(
                f"{d('❌ Please ')}/login{d(' first to use your Telegram account.\n\n')}"
                f"{d('💬 Support: ')}{url}",
                parse_mode="HTML",
            )
            return False
...
async def show_approval_screen(message: Message):
    """Send the expired-trial screen with contact info for purchase."""
    rec = await get_user_record(message.chat.id)
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💬 Buy Approval — Contact @ItsJayHong",
                    url="https://t.me/ItsJayHong",
                )
            ]
        ]
    )
    ...
    text_lines = [
        f"⏰ <b>Your 1-hour free trial has expired!</b>",
        "",
        f"To continue using <b>{DENIAL_FONT} Spreader Bot</b>, you need approval.",
        "",
        f"💬 Contact <b>{ADMIN_USERNAME}</b> to buy access:",
        f"👉 https://t.me/ItsJayHong",
        "",
    ]
    join_channels = await get_join_list()
    if join_channels:
        text_lines.append(f"📢 You must also join: {', '.join(join_channels)}")
    if rec and rec["status"] == UserStatus.PENDING.value:
        text_lines.append("\n⏳ Your approval request is already <b>pending</b>. Admin will review soon.")
...
# ----------------------------------------------------------------------
# Helper: build admin timed-access keyboard
# ----------------------------------------------------------------------
def _make_admin_access_kb(cid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Full Access", callback_data=f"adm_approve:{cid}"),
                InlineKeyboardButton(text="🕐 1 Hour",     callback_data=f"adm_time:{cid}:1"),
                InlineKeyboardButton(text="📅 1 Day",      callback_data=f"adm_time:{cid}:24"),
            ],
            [
                InlineKeyboardButton(text="📆 7 Days",     callback_data=f"adm_time:{cid}:168"),
                InlineKeyboardButton(text="❌ Reject",     callback_data=f"adm_reject:{cid}"),
                InlineKeyboardButton(text="🚫 Ban",        callback_data=f"adm_ban:{cid}"),
            ],
        ]
    )
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if cid == ADMIN_ID:
        await message.answer_photo(
            ...
            caption=(
                ...
                f"{d('/approve &lt;id&gt; - Full access\n')}"
                f"{d('/approve1h &lt;id&gt; - 1 hour access\n')}"
                f"{d('/approve1d &lt;id&gt; - 1 day access\n')}"
                f"{d('/approve7d &lt;id&gt; - 7 days access\n')}"
                f"{d('/reject &lt;id&gt; - Reject user\n')}"
                f"{d('/ban &lt;id&gt; - Ban user\n')}"
                f"{d('/unban &lt;id&gt; - Unban user\n')}"
                f"{d('/broadcast &lt;msg&gt; - Send to all users\n')}"
                f"{d('/stats - View bot statistics\n')}"
                f"{d('/addjoin @ch - Add must-join channel\n')}"
                f"{d('/removejoin @ch - Remove from join list\n')}"
                f"{d('/listjoin - Show all required joins\n\n')}"
                f"{d('User commands:\n')}"
    ...
    # Auto-grant 1H free trial for brand-new users (no button click needed)
    is_new_user = not rec or rec["status"] == UserStatus.NONE.value
    if is_new_user:
        trial_until = (
        ...
        rec = await get_user_record(cid)
        # Notify admin with timed-access buttons
        try:
            await bot.send_message(
                ADMIN_ID,
                f"👤 <b>New user started bot</b>\n\n"
                f"Name: {html.quote(message.from_user.full_name)}\n"
                f"ID: <code>{cid}</code>\n"
                f"Username: {'@' + message.from_user.username if message.from_user.username else '(none)'}\n\n"
                f"🎁 Auto-granted 1H free trial. Grant access:",
                parse_mode="HTML",
                reply_markup=_make_admin_access_kb(cid),
            )
        except Exception:
            pass
    ...
    # Show trial-activated banner to new users before main menu
    if is_new_user:
        contact_kb = InlineKeyboardMarkup(
            inline_keyboard=[[
                InlineKeyboardButton(
                    text="💬 Contact Admin for Full Access",
                    url="https://t.me/ItsJayHong",
                )
            ]]
        )
        join_list = await get_join_list()
        join_text = "\n".join(f"• {ch}" for ch in join_list) if join_list else ""
        await message.answer_photo(
            photo=START_IMAGE_URL,
            caption=(
                f"🎁 <b>Your 1-Hour Free Trial is now ACTIVE!</b>\n\n"
                f"⏰ You have <b>1 hour</b> of free access to {DENIAL_FONT} Spreader Bot.\n\n"
                f"After your trial expires, contact the admin to get full access:\n"
                f"👉 {ADMIN_USERNAME}\n\n"
                + (f"📢 You must also join:\n{join_text}" if join_text else "")
            ),
            parse_mode="HTML",
    ...
    # Authorized: still check all required channels/groups
    all_joined, unjoined = await get_unjoined_channels(cid)
    if not all_joined:
        join_buttons = [
            [InlineKeyboardButton(
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=join_buttons)
        channels_text = "\n".join(
            f"• <b>{ch}</b>" for ch in unjoined
        )
        await message.answer_photo(
            photo=START_IMAGE_URL,
            caption=(
                f"{d('👋 Welcome back! You MUST join the following to use ')}"
                f"{DENIAL_FONT}{d(' Spreader Bot:\n\n')}"
                f"{channels_text}\n\n"
                f"{d('Then tap ')}<b>{d('I have joined all ✅')}</b>{d('.')}"
            ),
...
async def cb_request_approval(callback: CallbackQuery):
    ...
    try:
        await bot.send_message(
            ...
            f"{d('while pending. Grant access:')}",
            parse_mode="HTML",
            reply_markup=_make_admin_access_kb(cid),
        )
    except Exception as e:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(
                text="📢 Join " + REQUIRED_CHANNEL,
                url=REQUIRED_CHANNEL_URL)],
            [InlineKeyboardButton(
                text="✅ I've joined", callback_data="check_channel")],
        ]
    )
    ...
    if all_joined:
        try:
            ...
            await callback.message.answer(
                f"{d('✅ Join confirmed. You can now use ')}"
                f"{DENIAL_FONT}{d(' Spreader Bot. Type /help.')}",
                parse_mode="HTML",
            )
        await callback.answer("Verified ✅")
    else:
        missing = ", ".join(unjoined)
        await callback.answer(
            f"You still need to join: {missing}", show_alert=True
        )
...
async def cb_admin_approve(callback: CallbackQuery):
    ...
    try:
        start_kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🚀 Open Bot", url="https://t.me/SpreadingDBot?start=approved")
        ]])
        await bot.send_message(
            target,
            f"{d('🎉 Congratulations! You have been ')}<b>{d('approved')}</b>{d('!\n\n')}"
            f"{d('You now have ')}<b>{d('full permanent access')}</b>{d(' to ')}"
            f"{DENIAL_FONT}{d(' Spreader Bot.\n\n')}"
            f"{d('Tap the button below or type /start to begin.')}",
            parse_mode="HTML",
...
@dp.callback_query(F.data.startswith("adm_time:"))
async def cb_admin_timed(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("Admins only.", show_alert=True)
        return
    ...
    rec = await get_user_record(target)
    uname = f" (@{rec['username']})" if rec and rec.get("username") else ""
    label = f"{hours}h" if hours < 24 else f"{hours // 24}d"
    try:
        start_kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🚀 Open Bot", url="https://t.me/SpreadingDBot?start=access")
        ]])
        await bot.send_message(
            target,
            f"✅ <b>Access Granted — {label}!</b>\n\n"
            f"The admin has given you <b>{label}</b> of access to "
            f"{DENIAL_FONT} Spreader Bot.\n\n"
            f"Tap the button below or type /start to begin.\n\n"
            f"⏰ Your access expires in <b>{label}</b>.",
            parse_mode="HTML",
            reply_markup=start_kb,
        )
    except Exception:
    ...
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(
        f"✅ Granted <b>{label}</b> access to <code>{target}</code>{uname}.",
        parse_mode="HTML",
    )
    await callback.answer(f"Granted {label}.")
...
async def cmd_approve(message: Message):
    ...
    try:
        ...
        pass
    await message.answer(f"{d('✅ Approved ')}<code>{target}</code>{d('.')}", parse_mode="HTML")


async def _grant_timed(message: Message, hours: int):
    target = _admin_id_from_msg(message)
    label = f"{hours}h" if hours < 24 else f"{hours // 24}d"
    if not target:
        await message.answer(f"Usage: /{message.text.split()[0][1:]} <chat_id>")
        return
    expires_at = (
        datetime.now(timezone.utc) + timedelta(hours=hours)
    ).isoformat()
    ...
    try:
        ...
        pass
    await message.answer(
        f"✅ Granted <b>{label}</b> to <code>{target}</code>.", parse_mode="HTML"
    )


@dp.message(Command("approve1h"))
@admin_only
async def cmd_approve1h(message: Message):
    await _grant_timed(message, 1)


@dp.message(Command("approve1d"))
@admin_only
async def cmd_approve1d(message: Message):
    await _grant_timed(message, 24)


@dp.message(Command("approve7d"))
@admin_only
async def cmd_approve7d(message: Message):
    await _grant_timed(message, 168)
...
async def process_code(message: Message, state: FSMContext):
    cid = message.chat.id
    # Accept OTP with or without spaces: "12345" or "1 2 3 4 5"
    raw = message.text.strip()
    code = "".join(raw.split())  # removes all whitespace
    if not code.isdigit() or len(code) < 4:
        await message.answer(
            d("❌ Code must be numeric (5-6 digits).\n")
             + d("💡 Tip: send it like ")
             + d("1 2 3 4 5 6")
             + d(" (with spaces) or ")
             + d("123456")
...
async def cmd_send_inbox(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"⏱ {d('Auto delay: 3–5 seconds between each DM to avoid bans.\n\n')}"
...
# ----------------------------------------------------------------------
# /setchannel - admin can change the required channel at any time
# ----------------------------------------------------------------------
...
async def cmd_setchannel(message: Message):
    ...
    if len(parts) < 2:
        cur = await get_required_channel()
        url = await get_required_channel_url()
        await message.answer(
            f"⚙️ Current required channel: <b>{cur}</b>\n"
            f"🔗 URL: <code>{url}</code>\n\n"
            f"Usage:\n"
            f"/setchannel @username\n"
            f"/setchannel https://t.me/username\n"
            f"/setchannel https://t.me/+invitehash",
            parse_mode="HTML",
        )
        return
    new_ch = parts[1].strip()
    uname, url = normalize_channel(new_ch)
    await set_setting("required_channel", uname or new_ch)
    await message.answer(
        f"✅ Required channel updated!\n\n"
        f"Channel: <b>{uname or new_ch}</b>\n"
        f"URL: <code>{url}</code>\n\n"
        f"All users will now be required to join this channel.",
        parse_mode="HTML",
    )


# ----------------------------------------------------------------------
# /addjoin, /removejoin, /listjoin — manage multi-channel join list
# ----------------------------------------------------------------------
@dp.message(Command("addjoin"))
@admin_only
async def cmd_addjoin(message: Message):
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip():
        await message.answer(
            "📢 <b>Usage:</b> /addjoin @channel\n\n"
            "Adds a channel or group to the must-join list.\n"
            "Users will be required to join it before using the bot.\n\n"
            "<b>Example:</b> /addjoin @JayHCode",
            parse_mode="HTML",
        )
        return
    ch = parts[1].strip()
    uname, url = normalize_channel(ch)
    added = await add_to_join_list(uname or ch)
    if added:
        await message.answer(
            f"✅ <b>Added to join list:</b> {uname or ch}\n"
            f"URL: <code>{url}</code>\n\n"
        ...
        await message.answer(
            f"⚠️ <b>{uname or ch}</b> is already in the join list.",
            parse_mode="HTML",
        )


@dp.message(Command("removejoin"))
@admin_only
async def cmd_removejoin(message: Message):
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip():
        lst = await get_join_list()
        formatted = "\n".join(f"• {c}" for c in lst) if lst else "(empty)"
        await message.answer(
            f"📢 <b>Usage:</b> /removejoin @channel\n\n"
            f"Current join list:\n{formatted}",
            parse_mode="HTML",
        )
        return
    ch = parts[1].strip()
    uname, _ = normalize_channel(ch)
    removed = await remove_from_join_list(uname or ch)
    if removed:
        await message.answer(
            f"✅ <b>Removed from join list:</b> {uname or ch}",
            parse_mode="HTML",
        )
    else:
        await message.answer(
            f"⚠️ <b>{uname or ch}</b> was not found in the join list.",
            parse_mode="HTML",
        )


@dp.message(Command("listjoin"))
@admin_only
async def cmd_listjoin(message: Message):
    lst = await get_join_list()
    if not lst:
        await message.answer("📋 The join list is <b>empty</b>.", parse_mode="HTML")
        return
    lines = "\n".join(
        f"{i+1}. {ch}  —  {normalize_channel(ch)[1]}" for i, ch in enumerate(lst)
    )
    await message.answer(
        f"📋 <b>Required join list ({len(lst)} entries):</b>\n\n{lines}\n\n"
        f"Use /addjoin or /removejoin to manage.",
        parse_mode="HTML",
    )


# ----------------------------------------------------------------------
# /broadcast - admin sends a message to every user who started the bot
# ----------------------------------------------------------------------
...
async def cmd_broadcast(message: Message):
    ...
    if len(parts) < 2 or not parts[1].strip():
        await message.answer(
            "📢 Usage:\n"
            "/broadcast <message>\n\n"
            "Sends the message to ALL users who have ever started the bot."
        )
        return
    text = parts[1]
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT chat_id FROM approvals WHERE status != 'banned'"
        ) as cur:
...
# ----------------------------------------------------------------------
# /stats - admin views bot statistics
# ----------------------------------------------------------------------
@dp.message(Command("stats"))
@admin_only
async def cmd_stats(message: Message):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM approvals") as cur:
        ...
        async with db.execute(
            "SELECT COUNT(*) FROM approvals WHERE status = 'trial' "
    ...
    await message.answer(
        f"📊 <b>{DENIAL_FONT} Spreader Bot — Stats</b>\n\n"
        f"👥 Total users: <b>{total_users}</b>\n"
        f"✅ Approved: <b>{approved}</b>\n"
        f"🎁 Trial (total): <b>{trial}</b>\n"
        f"🟢 Active trials: <b>{active_trials}</b>\n"
...
# ----------------------------------------------------------------------
# /send_contacts - DM every Telegram contact of the user's account
# ----------------------------------------------------------------------


async def get_contact_entities(client: TelegramClient) -> list:
    """Return Telegram User entities for every contact (best-effort)."""
    result = await client(GetContactsRequest(hash=0))
    return list(result.users)


async def run_contacts_broadcast(chat_id: int, message_text: str, delay: float):
    client = None
    try:
        client = await get_authorized_client(chat_id)
        if not client:
            await bot.send_message(
                chat_id, "❌ Session lost. Contacts broadcast stopped."
            )
            return
        try:
            users = await get_contact_entities(client)
        except Exception as e:
            await bot.send_message(
                chat_id, f"❌ Could not list contacts: {str(e)[:120]}"
            )
            return
        # Filter out bots / self / deleted accounts
        ...
        targets = [
            ...
            and getattr(u, "id", None) != me_id
        ]
        if not targets:
            await bot.send_message(
                chat_id,
                "❌ No contacts with usable accounts found.",
            )
            return
        await bot.send_message(
            chat_id,
            f"📞 Sending DM to {len(targets)} contact(s)...",
        )
        sent, failed = 0, 0
        for idx, ent in enumerate(targets):
            try:
                await client.send_message(ent, message_text)
                sent += 1
            except FloodWaitError as e:
                await bot.send_message(
                    chat_id, f"⏳ Flood wait sleeping {e.seconds}s"
                )
                await asyncio.sleep(e.seconds)
                try:
                    await client.send_message(ent, message_text)
        ...
        try:
            await bot.send_message(chat_id, f"❌ Contacts broadcast crashed: {e}")
        except Exception:
            pass
    finally:
        if client and chat_id not in user_clients:
...
@dp.message(Command("send_contacts"))
async def cmd_send_contacts(message: Message, state: FSMContext):
    if not await gate(message):
        return
    cid = message.chat.id
    await state.set_state(ContactsBroadcastStates.waiting_message)
    await message.answer(
        f"📞 <b>Contacts broadcast</b>\n\n"
        f"Sends a DM to <b>every contact</b> in your Telegram phone book.\n\n"
        f"Send the message you want to deliver.\n"
        f"Use /cancel to abort.",
        parse_mode="HTML",
    )


@dp.message(ContactsBroadcastStates.waiting_message, F.text)
async def process_contacts_message(message: Message, state: FSMContext):
    text = message.text.strip()
    if not text:
        await message.answer(d("❌ Message cannot be empty."))
        return
    await state.update_data(message=text)
    await state.set_state(ContactsBroadcastStates.waiting_delay)
    await message.answer(
        d("✅ Message received.\n\n")
        + d("Now send the delay in seconds between DMs.\n")
        + d("Recommended: 5–15 seconds.")
    )


@dp.message(ContactsBroadcastStates.waiting_delay, F.text)
async def process_contacts_delay(message: Message, state: FSMContext):
    cid = message.chat.id
    try:
        delay = float(message.text.strip())
        if delay < 0:
    ...
    await state.clear()
    if not text:
        await message.answer(d("❌ Session lost. /send_contacts again."))
        return
    task = asyncio.create_task(run_contacts_broadcast(cid, text, delay))
    await message.answer(
        f"🚀 <b>Contacts broadcast STARTED</b>\n\n"
        f"Targets: ALL your Telegram contacts\n"
        f"Delay: {delay}s",
        parse_mode="HTML",
    )
...
# ----------------------------------------------------------------------
# Catch-all handler - responds to any unknown message from a user
# ----------------------------------------------------------------------
@dp.message()
async def catch_all(message: Message, state: FSMContext):
    """Last-resort handler so non-admin users always get a reply."""
    cid = message.chat.id
    if cid == ADMIN_ID:
        return  # admin's stray messages are ignored
    rec = await get_user_record(cid)
    if is_banned(rec):
        await message.answer(
            d("🚫 You are banned from this bot."),
            parse_mode="HTML",
        )
        return
    if not is_authorized(rec):
    ...
    if not await user_joined_channel(cid):
        ...
        await message.answer_photo(
            photo=START_IMAGE_URL,
            caption=(
                f"{d('🔒 You MUST join ')}<b>{ch}</b>{d(' first!')}\n\n"
                f"{d('📢 Join here: ')}{url}"
            ),
            parse_mode="HTML",
            reply_markup=kb,
        ...
        return
    # Authorized + joined: just send help hint
    await message.answer(
        f"{d('🤖 I didn\'t understand that.')}\n\n"
        f"{d('Type ')}/help{d(' to see the full command list.')}",
        parse_mode="HTML",
    )
